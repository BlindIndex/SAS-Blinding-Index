	...
	*James BI;
	proc iml;
		...
		*check for any missing or negative value in ANCILLARY;
		...
		if maxC > 0 | minR < 0 | maxCa > 0 | minRa < 0 then
					...
					do;
						...
						if IsEmpty(JAMES_WEIGHTS) = 0 then
							do;
								...
								if nrowJAMES_WEIGHTS = 3 & ncolJAMES_WEIGHTS = 2 then
					...
					do;
						...
						if IsEmpty(JAMES_WEIGHTS) = 0 then
							do;
								...
								if nrowJAMES_WEIGHTS = 4 & ncolJAMES_WEIGHTS = 3 then
									Weights = JAMES_WEIGHTS;
								else
									do;
										print 'ERROR: James weights have wrong structure';
							...
							do;
								...
								if IsEmpty(BANG_WEIGHTS) = 0 then
									do;
										...
										if nrowBANG_WEIGHTS = 6 & ncolBANG_WEIGHTS = 1 & BANG_WEIGHTScolTot = 0 then
											Weights = BANG_WEIGHTS;
										else stop 'ERROR: Bang weights have wrong structure or their sum is not zero';
									end;
							...
							do;
								...
								if IsEmpty(BANG_WEIGHTS) = 0 then
									do;
										...
										if nrowBANG_WEIGHTS = 5 & ncolBANG_WEIGHTS = 1 & BANG_WEIGHTScolTot = 0 then
											Weights = BANG_WEIGHTS;
										else stop 'ERROR: Bang weights have wrong structure or their sum is not zero';
									end;
...
/*ods _all_ close; ods listing; options nodate nonumber pagesize=60 linesize=120 FORMCHAR='|_ _ ||||||+=|-/\<>*';*/
...
*James: Unblinding message for two-sided CL;
%BI(%str({
179 0,	
41 89,	
76 38
